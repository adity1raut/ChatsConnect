...
import {
  ...
  Users, AtSign, Calendar, Wifi, WifiOff, Loader2,
} from "lucide-react";
...
export default function UserProfile() {
  ...
  const { friends, incomingRequests, sentRequests, sendRequest, acceptRequest, rejectRequest, cancelRequest, removeFriend, getRelationship } = useFriends();
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      setLoading(true);
      try {
        const { data } = await axios.get(`${API_URL}/profile/${userId}`, { headers: authHeader() });
  ...
  const handleSendRequest = async () => {
    ...
    try { await sendRequest(userId); } finally { setActionLoading(false); }
  };
  ...
  const handleAccept = async () => {
    ...
    if (!req) return;
    setActionLoading(true);
    try { await acceptRequest(req._id, userId); } finally { setActionLoading(false); }
  };
  ...
  const handleCancel = async () => {
    ...
    if (!req) return;
    setActionLoading(true);
    try { await cancelRequest(req._id, userId); } finally { setActionLoading(false); }
  };

  const handleRemove = async () => {
    setActionLoading(true);
    try { await removeFriend(userId); } finally { setActionLoading(false); }
  };
  ...
  if (loading) {
    return (
      <div className={`min-h-[100dvh] flex items-center justify-center ${isDark ? "bg-[#0a0a14]" : "bg-[#f4f5ff]"}`}>
  ...
  if (error || !profile) {
    return (
      <div className={`min-h-[100dvh] flex flex-col items-center justify-center gap-4 ${isDark ? "bg-[#0a0a14] text-white" : "bg-[#f4f5ff] text-gray-900"}`}>
        <p className="text-lg font-semibold">{error || "User not found"}</p>
        <button onClick={() => navigate(-1)} className="text-violet-500 hover:underline text-sm flex items-center gap-1">
  ...
  return (
    <div className={`min-h-[100dvh] relative overflow-hidden transition-all duration-700 ${isDark ? "bg-[#0a0a14] text-gray-50" : "bg-[#f4f5ff] text-gray-900"}`}>
      {/* Background */}
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 py-6 sm:py-10">
        ...
        {/* Profile card */}
        <div className={`rounded-3xl backdrop-blur-2xl shadow-2xl overflow-hidden ${isDark ? "bg-gray-900/70 border border-white/5" : "bg-white/80 border border-white/80"}`}>
          <div className="h-1 w-full bg-gradient-to-r from-violet-500 via-purple-500 to-pink-500" />

          {/* Cover gradient */}
          <div
            className="h-28 w-full"
            style={{ background: "linear-gradient(135deg, #7c3aed 0%, #a855f7 50%, #ec4899 100%)", opacity: 0.85 }}
          />
          ...
          <div className="px-6 pb-6">
            <div className="flex items-end justify-between -mt-12 mb-4">
              <div className="relative">
                ...
                {/* Online dot */}
                <div className={`absolute bottom-1 right-1 z-20 w-4 h-4 rounded-full border-2 ${profile.isOnline ? "bg-emerald-500" : "bg-gray-400"} ${isDark ? "border-gray-900" : "border-white"}`} />
              </div>
                ...
                <div className="flex gap-2 flex-wrap justify-end">
                  ...
                  {isFriend ? (
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <button
                        onClick={() => { const req = incomingRequests.find(r=>r.sender._id===userId); if(req) rejectRequest(req._id, userId); }}
                        disabled={actionLoading}
            ...
            <div className="mb-4">
              ...
              <p className={`flex items-center gap-1 text-sm mt-1 ${isDark ? "text-gray-400" : "text-gray-500"}`}>
            ...
            {/* Bio */}
            {profile.bio && (
              <p className={`text-sm leading-relaxed mb-5 italic ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                "{profile.bio}"
              </p>
            )}

            {/* Stats row */}
            <div className={`flex gap-4 py-4 border-y ${isDark ? "border-white/6" : "border-gray-100"}`}>
              <div className="flex items-center gap-2">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isDark ? "bg-violet-500/15" : "bg-violet-50"}`}>
                  <Users size={16} className="text-violet-500" />
                </div>
                <div>
                  <p className={`text-lg font-bold leading-none ${isDark ? "text-white" : "text-gray-900"}`}>{friendCount}</p>
                  <p className={`text-xs mt-0.5 ${isDark ? "text-gray-500" : "text-gray-400"}`}>Friends</p>
                </div>
              </div>

              <div className="flex items-center gap-2">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${profile.isOnline ? isDark ? "bg-emerald-500/15" : "bg-emerald-50" : isDark ? "bg-white/4" : "bg-gray-100"}`}>
                  {profile.isOnline
                    ? <Wifi size={16} className="text-emerald-500" />
                    : <WifiOff size={16} className={isDark ? "text-gray-500" : "text-gray-400"} />}
                </div>
                <div>
                  <p className={`text-sm font-bold leading-none ${profile.isOnline ? "text-emerald-500" : isDark ? "text-gray-400" : "text-gray-500"}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className={`text-xs mt-0.5 ${isDark ? "text-gray-500" : "text-gray-400"}`}>Joined</p>
                </div>
              </div>
            </div>
          </div>
