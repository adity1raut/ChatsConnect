...
export default function DeleteModal({ isDark, deleteConfirmText, setDeleteConfirmText, loading, onConfirm, onCancel }) {
  ...
  return (
    ...
    >
      <div
        className={`relative w-full max-w-[440px] rounded-3xl shadow-2xl overflow-hidden transition-all ${isDark
      ...
      >
        ...
        <div className="p-8 sm:p-10">
          {/* Close button */}
          <button
            className={`absolute top-5 right-5 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 ${isDark ? "text-gray-500 hover:bg-white/10 hover:text-white" : "text-gray-400 hover:bg-gray-100 hover:text-gray-700"
              }`}
            onClick={onCancel}
          >
            <X size={16} />
          ...
          {/* Icon */}
          <div
            className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 relative ${isDark ? "bg-red-500/15" : "bg-red-50"
              }`}
          >
            <div className="absolute inset-0 rounded-2xl bg-red-500/10 animate-ping" style={{ animationDuration: "2s" }} />
            <AlertTriangle size={26} className="text-red-500 relative z-10" strokeWidth={2} />
          </div>

          <h3 className={`text-2xl font-bold mb-2 tracking-tight ${isDark ? "text-white" : "text-gray-900"}`}>
            Delete your account?
          </h3>
          <p className={`text-sm leading-relaxed mb-6 ${isDark ? "text-gray-400" : "text-gray-500"}`}>
            This action is <strong className="text-red-400">permanent and irreversible</strong>. All your data, messages, and settings will be permanently deleted. Type{" "}
            <span
              className={`inline font-mono font-bold px-1.5 py-0.5 rounded text-red-500 ${isDark ? "bg-red-500/15" : "bg-red-50"
                }`}
            >
              DELETE
            </span>{" "}
          ...
          <div className="relative mb-6">
            <input
              ...
              className={`w-full px-4 py-3.5 text-sm font-mono rounded-xl border outline-none transition-all duration-200 ${isDark
                  ? `bg-white/[0.04] text-white placeholder-gray-600 ${isConfirmed
                    ? "border-red-500/60 ring-4 ring-red-500/10"
                    : "border-white/8 focus:border-red-500/60 focus:ring-4 focus:ring-red-500/10"
                  }`
                  : `bg-gray-50 text-gray-900 placeholder-gray-400 ${isConfirmed
          ...
          <div className="flex gap-3">
            <button
              onClick={onCancel}
              className={`flex-1 py-3 rounded-xl font-semibold text-sm transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] ${isDark
