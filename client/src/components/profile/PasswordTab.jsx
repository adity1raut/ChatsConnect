  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="relative group">
        <Lock
          className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none transition-colors duration-200 ${isDark ? "text-gray-600 group-focus-within:text-emerald-400" : "text-gray-400 group-focus-within:text-emerald-500"
            }`}
        />
        <input
          ...
          className={`w-full pl-11 pr-12 py-3.5 text-sm rounded-xl border outline-none transition-all duration-200 ${isDark
        ...
        <button
          type="button"
          onClick={onToggle}
          className={`absolute right-4 top-1/2 -translate-y-1/2 transition-all duration-200 hover:scale-110 ${isDark ? "text-gray-600 hover:text-gray-300" : "text-gray-400 hover:text-gray-600"
            }`}
        >
...
const strengthConfig = [
  null,
  { label: "Weak", color: "#ef4444", bg: "rgba(239,68,68,0.12)", text: "text-red-500", tip: "Use a longer password" },
  { label: "Fair", color: "#f97316", bg: "rgba(249,115,22,0.12)", text: "text-orange-500", tip: "Add uppercase letters" },
  { label: "Good", color: "#3b82f6", bg: "rgba(59,130,246,0.12)", text: "text-blue-500", tip: "Add symbols for stronger" },
  { label: "Strong", color: "#22c55e", bg: "rgba(34,197,94,0.12)", text: "text-emerald-500", tip: "Great password!" },
];
          ...
          >
            <div className="flex items-center justify-between mb-2.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-xs font-bold tracking-wide ${sc?.text}`}>{sc?.label}</span>
              </div>
              <span className={`text-[11px] ${isDark ? "text-gray-500" : "text-gray-400"}`}>{sc?.tip}</span>
            </div>
            <div className="flex gap-1.5">
              {[1, 2, 3, 4].map((i) => (
                <div
                  ...
                  style={{
                    backgroundColor: i <= strength ? sc?.color : isDark ? "#1f2937" : "#e5e7eb",
