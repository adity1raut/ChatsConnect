...
import {
  ...
  Users, Star, Zap, MessageSquare, Check, X,
} from "lucide-react";
...
const TYPE_CFG = {
  ...
  message:         { icon: MessageSquare,  iconClass: "text-violet-400",              bg: "from-violet-500 to-purple-600", soft: "bg-violet-500/15" },
  groupMessage:    { icon: Users,          iconClass: "text-blue-400",                bg: "from-blue-500 to-cyan-500",     soft: "bg-blue-500/15" },
  friendRequest:   { icon: UserPlus,       iconClass: "text-emerald-400",             bg: "from-emerald-500 to-teal-500",  soft: "bg-emerald-500/15" },
  friendAccepted:  { icon: Check,          iconClass: "text-emerald-400",             bg: "from-emerald-500 to-teal-500",  soft: "bg-emerald-500/15" },
};
...
const ACTIVITY_STATS_CFG = [
  { label: "Total",    key: "total",   icon: Bell,          color: "from-violet-500 to-purple-600" },
  { label: "Unread",   key: "unread",  icon: Zap,           color: "from-rose-500 to-pink-500" },
  { label: "Messages", key: "message", icon: MessageSquare, color: "from-blue-500 to-cyan-500" },
  { label: "Groups",   key: "group",   icon: Users,         color: "from-emerald-500 to-teal-500" },
];
...
export default function NotificationPage() {
  ...
  const filtered = notifications.filter((n) => {
    ...
    if (activeFilter === "friendRequest") return n.type === "friendRequest" || n.type === "friendAccepted";
  ...
  const handleAccept = async (e, n) => {
    ...
    try {
      ...
      setFriendActionLoading((prev) => { const next = { ...prev }; delete next[n.id]; return next; });
    }
  };
    ...
    if (n.type === "message" && n.chatId) {
      navigate("/chat", {
        state: {
          ...
          openChat: { id: n.from._id, name: n.from.name, username: n.from.username, avatar: n.from.avatar, type: "user" },
        },
      });
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 mb-6 sm:mb-8"
          ...
          {ACTIVITY_STATS_CFG.map((s) => {
            ...
            return (
              <div key={s.label} className={`relative rounded-2xl p-4 sm:p-5 border overflow-hidden transition-all hover:scale-[1.02] hover:-translate-y-0.5 cursor-default ${card}`}>
                ...
                {FILTER_TABS.map(({ id, label, icon: Icon }) => {
                  ...
                  return (
                    <button key={id} onClick={() => setActiveFilter(id)}
                      ...
                      {id === "friendRequest" && (() => {
                        const pendingCount = notifications.filter((n) => n.type === "friendRequest" && !n.read).length;
            ...
            {filtered.length > 0 ? (
              <div className="divide-y divide-white/[0.035]">
                {filtered.map((n) => {
                  ...
                  const Icon = cfg.icon;
                  const isMsg = n.type === "message" || n.type === "groupMessage";
                  const isFriend = n.type === "friendRequest" || n.type === "friendAccepted";
                    ...
                    <div key={n.id}
                      ...
                      <div className="relative shrink-0">
                        {n.from?.avatar && n.from.avatar.startsWith("http") ? (
                          <img src={n.from.avatar} alt="" className="w-11 h-11 rounded-full object-cover shadow-md" />
                        ) : (
                          <div className="w-11 h-11 rounded-full flex items-center justify-center text-lg shadow-md text-white font-bold"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm ${isDark ? "text-gray-200" : "text-gray-800"}`}>
                          <span className="font-bold">{n.from?.name || "Someone"}</span>{" "}
                          <span className={isDark ? "text-gray-400" : "text-gray-500"}>
                            {n.type === "message" ? "sent you a message"
                              : n.type === "groupMessage" ? "sent a group message"
                              : n.type === "friendRequest" ? "sent you a friend request"
                              : n.type === "friendAccepted" ? "accepted your friend request"
                              : n.content || ""}
                          </span>
                        </p>
                        ...
                        <p className={`text-[10px] mt-1 font-medium ${isDark ? "text-gray-600" : "text-gray-400"}`}>
                          {timeAgo(n.time)}
                        </p>
                        {isMsg && (
                        ...
                        {n.type === "friendRequest" && (
                          <div className="flex gap-2 mt-2.5">
                            <button onClick={(e) => handleAccept(e, n)}
                              ...
                              {friendActionLoading[n.id] === "accept" ? "Accepting…" : "Accept"}
                            </button>
                            <button onClick={(e) => handleReject(e, n)}
                              ...
                              {friendActionLoading[n.id] === "reject" ? "Rejecting…" : "Decline"}
                            </button>
                          </div>
                        )}
                        {n.type === "friendAccepted" && (
                          <div className="mt-2">
                            <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold ${isDark ? "bg-emerald-500/15 text-emerald-400" : "bg-emerald-50 text-emerald-600"}`}>
                      ...
                      {!n.read && <div className="shrink-0 w-2 h-2 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 mt-2.5 shadow-sm shadow-violet-500/40" />}
                    </div>
                  );
                })}
              </div>
            ...
            <div className={`rounded-2xl border overflow-hidden ${card}`}>
              ...
              <div className="p-5">
                ...
                <div className="flex flex-col gap-2">
                  {[
                    { label: "Mark all read", icon: CheckCheck, color: "from-violet-500 to-purple-600", action: markAllRead },
                    { label: "Clear filter", icon: Filter, color: "from-blue-500 to-cyan-500", action: () => setActiveFilter("all") },
                    { label: "Go to chat", icon: MessageSquare, color: "from-emerald-500 to-teal-500", action: () => navigate("/chat") },
                    { label: "Settings", icon: Settings, color: "from-orange-500 to-amber-500", action: () => {} },
                    ...
                    <button key={label} onClick={action}
                      ...
                      <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center transition-transform group-hover:scale-110 shadow`}>
              ...
              <div className="relative z-10 p-5">
                ...
                <p className="text-white/90 text-sm leading-relaxed mb-4">
                  {notifications.length === 0
                    ...
                    : <>You have <span className="font-bold text-white">{notifications.length} notification{notifications.length !== 1 ? "s" : ""}</span> from messages.</>
                  }
                </p>
                <div className="flex flex-col gap-2">
                  {[
                    { label: "Direct messages", val: notifications.filter((n) => n.type === "message").length },
                    { label: "Group messages", val: notifications.filter((n) => n.type === "groupMessage").length },
