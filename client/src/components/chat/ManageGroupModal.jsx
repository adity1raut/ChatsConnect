...
export default function ManageGroupModal({
  ...
  return (
    ...
    >
      <div
        className={`w-full max-w-md rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] ${isDark ? "bg-gray-900 border border-white/8" : "bg-white border border-gray-100"}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2
                className={`font-bold text-base leading-none ${isDark ? "text-white" : "text-gray-900"}`}
              >
                Manage Members
              </h2>
              <p
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-5 min-h-0">
          ...
          {isAdmin && (
            <div>
              ...
              <div
                className={`relative flex items-center rounded-xl border mb-2 ${isDark ? "bg-white/4 border-white/8 focus-within:border-violet-500/40" : "bg-gray-50 border-gray-200 focus-within:border-violet-400"}`}
              >
                <Search
                  className={`absolute left-3.5 w-4 h-4 ${isDark ? "text-gray-600" : "text-gray-400"}`}
                />
                <input
                  type="text"
                  value={searchQuery}
              ...
              {searchResults.length > 0 && (
                <div
                  className={`rounded-xl border overflow-hidden ${isDark ? "border-white/8" : "border-gray-100"}`}
                >
                  {searchResults.map((u) => {
                    ...
                    return (
                      <div
                        key={u._id}
                        className={`flex items-center gap-3 px-3 py-2.5 ${isDark ? "hover:bg-white/4" : "hover:bg-gray-50"}`}
                      >
