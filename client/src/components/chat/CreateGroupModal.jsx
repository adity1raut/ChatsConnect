...
export default function CreateGroupModal({ onClose, onGroupCreated }) {
  ...
  const searchUsers = useCallback(async (q) => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/profile/search?query=${encodeURIComponent(q)}`, {
      ...
      <div className={`w-full max-w-md rounded-2xl border shadow-2xl ${base}`}>
        ...
        {step === 2 && (
          <div className="p-5 space-y-3">
            ...
            {/* Search input */}
            <div className={`relative flex items-center rounded-xl border ${isDark ? "bg-white/[0.04] border-white/10" : "bg-gray-50 border-gray-200"}`}>
              {loading ? (
            ...
            {/* Results */}
            <div className={`rounded-xl border max-h-48 overflow-y-auto ${isDark ? "border-white/[0.06]" : "border-gray-100"}`}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs truncate ${isDark ? "text-gray-500" : "text-gray-400"}`}>@{u.username}</p>
                      </div>
                      {isSelected && (
